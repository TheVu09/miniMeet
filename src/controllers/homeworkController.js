...
const createHomework = async (req, res) => {
  try {
    ...
    console.log('Creating homework:', { title, classId, teacher: req.user._id });
...
const getHomework = async (req, res) => {
  try {
    ...
    const isTeacher = homework.teacher._id.toString() === req.user._id.toString();
...
const listHomework = async (req, res) => {
  try {
    ...
    if (req.user.role === 'teacher') {
      ...
      const Class = require('../models/Class');
      const userClasses = await Class.find({ students: req.user._id }).select('_id');
